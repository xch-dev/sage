...
pub async fn remove_runtime_id_by_app_id(apps_state: &State<'_, AppsHostState>, app_id: &str) {
    let mut runtime_id_by_app_id = apps_state.runtime.runtime_id_by_app_id.lock().await;
    runtime_id_by_app_id.remove(app_id);
}

pub async fn remove_runtime_by_runtime_id(apps_state: &State<'_, AppsHostState>, runtime_id: &str) {
...
pub async fn remove_pending_stop_ready(apps_state: &State<'_, AppsHostState>, request_id: &String) {
