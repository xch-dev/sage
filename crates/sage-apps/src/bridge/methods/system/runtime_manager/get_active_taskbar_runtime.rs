...
use crate::runtime::{find_active_taskbar_runtime_id, find_runtime_by_runtime_id_optional, resolve_running_app, SageAppRuntimeRecordView};
...
impl BridgeMethod for RuntimeManagerGetActiveTaskbarRuntime {
    ...
    async fn handle(
        ...
        let active_runtime_id = find_active_taskbar_runtime_id(tools.host_state, &host_window_label).await;

        let Some(active_runtime_id) = active_runtime_id else {
            return Ok(Box::new(None::<SageAppRuntimeRecordView>));
        };

        let runtime: Option<SageAppRuntimeRecordView> = find_runtime_by_runtime_id_optional(tools.host_state, &active_runtime_id).await
            .map(Into::into);
