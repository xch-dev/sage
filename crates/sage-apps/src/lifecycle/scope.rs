...
async fn app_is_enabled_for_scope(
    app_state: &State<'_, AppState>,
    app: &SharedSageApp,
) -> bool {
    ...
    app.with(|sage_app| {
        wallet_scope_allows_fingerprint(
            sage_app.common().wallet_scope(),
            fingerprint,
        )
    })
}

fn wallet_scope_allows_fingerprint(
...
async fn current_wallet_fingerprint(
    app_state: &State<'_, AppState>,
) -> Option<u32> {
