...
use sage_api::{GetUserThemes, GetUserThemesResponse, SaveUserTheme, SaveUserThemeResponse, GetNftData, GetUserTheme, GetUserThemeResponse};
use tokio::fs;
...
impl Sage {
    pub async fn get_user_theme(&self, req: GetUserTheme) -> Result<GetUserThemeResponse> {
        ...
        Ok(GetUserThemeResponse { theme: Some(theme_json) })
    }
    ...
    pub async fn save_user_theme(&self, req: SaveUserTheme) -> Result<SaveUserThemeResponse> {
        ...
        let nft_data_response = self.get_nft_data(GetNftData { nft_id: req.nft_id.clone() }).await?;
