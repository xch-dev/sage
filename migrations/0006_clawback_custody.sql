...
FROM coins
  ...
  LEFT JOIN p2_puzzles AS sender_p2_puzzle
    ON sender_p2_puzzle.hash = clawbacks.sender_puzzle_hash
    AND sender_p2_puzzle.kind IN (0, 3)
  LEFT JOIN p2_puzzles AS receiver_p2_puzzle
    ON receiver_p2_puzzle.hash = clawbacks.receiver_puzzle_hash
    AND receiver_p2_puzzle.kind IN (0, 3)
