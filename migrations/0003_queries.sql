SELECT
  parent_coin_id, coins.puzzle_hash, amount
FROM coins
...
WHERE
  spent_height IS NULL AND assets.kind = 0
  AND (offers.id IS NULL OR offers.status != 0)
  AND transaction_coins.id IS NULL
  AND (
    clawbacks.id IS NULL
    OR (
      SELECT COUNT(*) FROM p2_puzzles WHERE p2_puzzles.hash = clawbacks.sender_puzzle_hash
      AND unixepoch() < clawbacks.seconds
    )
    OR (
      SELECT COUNT(*) FROM p2_puzzles WHERE p2_puzzles.hash = clawbacks.receiver_puzzle_hash
      AND unixepoch() >= clawbacks.seconds
    )
  )
  AND (
