...
class BarcodeScannerPlugin: Plugin, AVCaptureMetadataOutputObjectsDelegate {
    ...
    // Modify metadataOutput delegate method to use trigger instead of invoke
    public func metadataOutput(
        ...
        if scanFormats.contains(found.type) {
            var data: JSObject = [:]
            data["format"] = formatStringFromMetadata(found.type)
            data["content"] = found.stringValue ?? ""
            data["bounds"] = String(describing: found.bounds)

            // Trigger the event with the properly typed data
            trigger("barcode-detected", data: data)
        }
    }
    ...
    @objc private func startScan(_ invoke: Invoke) throws {
        ...
        DispatchQueue.main.async { [self] in
            ...
            invoke.resolve() // Acknowledge the start command
        }
    }

    @objc private func stopScan(_ invoke: Invoke) {
        destroy()
        trigger("barcode-stopped", data: [:])
